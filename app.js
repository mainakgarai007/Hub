...
async function requestJson(url,options={},attempt=0,provider='api'){
  ...
  try{
    ...
    if(e.name==='AbortError')throw new ApiError('Request timed out',408,provider);
    throw e;
...
async function requestAniList(mode,value=''){
  ...
  if(mode==='airing')mediaArgs='type: ANIME, status: RELEASING, isAdult: false, sort: NEXT_AIRING_EPISODE';
...
async function requestKitsu(path){
  ...
  const json=await requestJson(url,{},0,'Kitsu');
...
async function requestJikan(path){
  ...
  let jikanError=null;
  try{
    const json=await requestJson(`${API_BASE}${path}`,{},0,'Jikan');
    const data=Array.isArray(json.data)?json.data.map(normalizeJikan):[];
    if(data.length){cache.set(key,{time:Date.now(),data});return data;}
  ...
  const mode=isSearch?'search':path.includes('/top/anime')?'trending':'airing';
  const value=isSearch?params.get('q')||'':'';
  try{
    const data=await requestAniList(mode,value);
    if(data.length){cache.set(key,{time:Date.now(),data});return data;}
    throw new ApiError('AniList returned no results',204,'AniList');
  }catch(aniError){
    console.warn('AniList unavailable:',aniError);
    try{
      const data=await requestKitsu(path);
      if(data.length){cache.set(key,{time:Date.now(),data});return data;}
      throw new ApiError('Kitsu returned no results',204,'Kitsu');
    }catch(kitsuError){
      console.error('All anime APIs unavailable:',{jikanError,aniError,kitsuError});
      throw new ApiError('Anime providers are temporarily unavailable',503,'anime');
    }
  }
}

async function fetchCountdown(){
  const key='anime:countdown';
  const hit=cache.get(key);
  if(hit&&Date.now()-hit.time<CACHE_MS)return hit.data;
  try{
    const data=await requestAniList('airing');
    if(data.length){cache.set(key,{time:Date.now(),data});return data;}
  }catch(e){console.warn('AniList countdown unavailable:',e);}
  const data=await requestJikan('/anime?status=airing&order_by=popularity&sort=asc&limit=12');
  return data;
}
