...
async function startAccountUI(){
  ...
  const setImage=(el,url,show)=>{
    if(!el)return;
    if(show&&url){el.src=url;el.hidden=false}else{el.hidden=true;el.removeAttribute('src')}
  };
  const guest=()=>{
    button.href='login.html';
    text(name,'Login');
    setImage(avatar,null,false);
    if(fallback)fallback.hidden=false;
    setImage(menuAvatar,null,false);
    text(menuName,'Guest');
  ...
  const signedIn=user=>{
    ...
    if(user.photoURL){setImage(avatar,user.photoURL,true);if(fallback)fallback.hidden=true;setImage(menuAvatar,user.photoURL,true)}
    else{setImage(avatar,null,false);if(fallback)fallback.hidden=false;setImage(menuAvatar,null,false)}
...
startAccountUI().catch(error=>{console.error('Account UI failed:',error);const cta=document.getElementById('loginCta');if(cta)cta.hidden=false;});
